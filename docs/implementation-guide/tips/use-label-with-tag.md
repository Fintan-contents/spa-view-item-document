...
省力化コンポーネントは、1 つの画面用コンポーネントを配置することでラベルと入力コントロールを両方表示できます。
...
// 画面用コンポーネント1つでラベルと入力コントロールを表示できる
<AxInputText item={view.userName} />
```

![1つの画面用コンポーネントでラベルと入力コントロールを表示できる](../../../static/img/ax-component.png)
...
- 複数入力コントロールに対して 1 つのラベルをつける
- 装飾したラベルをつける

## 複数入力コントロールに対して 1 つのラベルをつける

`AxLabelWithTag` コンポーネントを使うことで、まとまりのある複数入力コントロールに対して 1 つのラベルをつけることができます。「名前」や「電話番号」といった、入力コントロールが複数に分かれる項目に対して有効です。

:::warning
`AxLabelWithTag` を使用する場合は、入力コントロールに対応する部品（例では `AxInputText` ）の `hideLabel` を `true` に設定する必要があります。
:::
