...
- API 呼び出し前後処理の実施  
  API 呼び出しの前後に行いたい処理（リクエストデータの整形やエラーハンドリングなど）をコールバック関数として渡すことができます。  
  前処理は boolean を返す関数で、true を返すと後続処理を継続、false を返すと処理を中断できます。  
  後処理は値を返さない関数（戻り値が void 型の関数）で、成功時用と失敗時用でコールバック関数を分けて指定します。
...
ボタンの内部処理は以下の順序で実施されます。なお、いずれかの処理でエラーが発生した場合、以降の処理は実施されません。

| 実行順 | 処理内容        | パラメータ                                  |
| ------ | ------------------- | ------------------------------------------- |
| 1      | バリデーション      | `validationViews`                           |
| 2      | 前処理              | `onBeforeApiCall`                           |
| 3      | API 呼び出し        | `event`                                     |
| 4      | 後処理（成功/失敗） | `onAfterApiCallSuccess/onAfterApiCallError` |
  ...
  return (
    <>
      ...
      <AxMutateButton
        ...
        onAfterApiCallSuccess={() => router.push(/top)} // API成功時に実施したい処理を渡す
