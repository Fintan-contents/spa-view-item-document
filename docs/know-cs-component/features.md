...
- API 呼び出し前後処理の実施  
  API 呼び出しの前後に行いたい処理（リクエストデータの整形やエラーハンドリングなど）をコールバック関数として渡すことができます。前処理は boolean を返す関数で、true を返すと後続処理を継続、false を返すと処理を中断できます。
  後処理は void 関数で、成功時用と失敗時用でコールバック関数を分けて指定します。
